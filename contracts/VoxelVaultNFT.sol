...
contract VoxelVaultNFT is ERC721, ERC721URIStorage, ERC721Royalty, Ownable {
    ...
    function mintTo(address recipient, address royaltyReceiver, string calldata uri, uint96 royaltyBps) external onlyOwner returns (uint256 tokenId) {
        return _mintTo(recipient, royaltyReceiver, uri, royaltyBps);
    }

    function _mintTo(address recipient, address royaltyReceiver, string calldata uri, uint96 royaltyBps) internal returns (uint256 tokenId) {
