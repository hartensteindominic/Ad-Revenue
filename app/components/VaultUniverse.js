...
const HERO = { name: 'Obsidian Dragon', family: 'creature', rarity: 'Mythic', material: 'crystal', shape: 'dragon', renderMode: 'voxel', seed: 'hero-obsidian-dragon', creator: 'Voxel Vault' };

function MediaFrame({ item, interactive = false, hero = false }) {
  const [failed, setFailed] = useState(false);
  const [retry, setRetry] = useState(0);
  const isVoxel = item?.renderMode === 'voxel';
  const common = {
    seed: item?.seed || `vault-${item?.id || 'item'}`,
  ...
  if (failed) {
    return (
      <div className="mediaFallback">
        <div className="fallbackCore" style={{ '--hue': (Number(item?.id || 1) * 47) % 360 }} />
        <span>3D PREVIEW RECOVERING</span>
        <button type="button" onClick={(event) => { event.stopPropagation(); setFailed(false); setRetry(value => value + 1); }}>Retry</button>
      </div>
    );
  }
...
export default function VaultUniverse() {
  ...
  const loadMore = useCallback(() => {
    ...
    setLoadedCount(value => Math.min(value + PAGE_SIZE, CATALOG_SIZE));
  }, [loadedCount]);
  ...
  async function payCrypto() {
    ...
    try {
      ...
      setStatus(error?.shortMessage || error?.reason || error?.message || 'Crypto purchase failed.');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function payCard() {
    ...
    try {
      ...
      setStatus(error?.message || 'Card checkout failed.');
      setBusy(false);
    }
  }
  ...
  return (
    <main className="vaultUniverse">
      ...
      <div className="microStars" aria-hidden="true" />

      <nav className="vaultNav">
        <a className="brand" href="/">V<span>V</span>OXELVAULT</a>
        <div className="navLinks">
          <a href="/discover">Discover</a><a href="/hunt">Hunt</a><a href="#drops">Gallery</a><a href="/trade">Trade</a><a href="/marketplace">Marketplace</a>
        </div>
      ...
      <section className="hero" id="discover">
        <div className="heroCopy">
          ...
          <div className="heroActions"><a className="primaryAction" href="/hunt">Start a scavenger job ↓</a><a className="secondaryAction" href="/discover">Map drops →</a></div>
        ...
        <div className="heroViewer">
          <MediaFrame item={HERO} hero />
          <div className="heroTag"><span>FEATURED · LIVE 3D · TAP TO INSPECT</span><strong>Obsidian Dragon</strong><small>Mythic · Crystal · Voxel Vault Original</small></div>
        </div>
      </section>
          ...
          {visible.map(item => (
            <article className="artCard" key={`${item.id}-${item.seed}`}>
              <button className="artVisual" onClick={() => openInspect(item)} aria-label={`Open ${item.name} in 3D`}>
                ...
                <span className="edition">{String(item.id).padStart(2, '0')}</span><span className="rarity">{item.rarity}</span><span className="inspectPill">↗ 3D</span>
              </button>
      ...
      <section className="compatibility" id="about"><div><div className="eyebrow">3D COLLECTIBLE STANDARD</div><h2>One object.<br /><em>Many worlds.</em></h2><p>Build assets that can be staged as GLB/GLTF, described with rich metadata and connected to wallet ownership. The same canonical asset can be prepared for future metaverse integrations without changing the collector's Vault experience.</p></div><div className="compatGrid"><div><b>GLB / GLTF</b><span>Portable 3D asset staging</span></div><div><b>WALLET</b><span>Ownership-aware collecting</span></div><div><b>METADATA</b><span>Traits, rarity & provenance</span></div><div><b>SANDBOX READY</b><span>Future publishing profile</span></div></div></section>
      ...
      {selected && <div className="inspectBackdrop" role="dialog" aria-modal="true" aria-label={`${selected.name} 3D inspection`} onClick={e => e.target === e.currentTarget && closeInspect()}>
        <div className="inspectModal"><button className="close" onClick={closeInspect}>×</button><div className="inspectViewer"><MediaFrame item={selected} interactive hero /></div><div className="inspectDetails">
