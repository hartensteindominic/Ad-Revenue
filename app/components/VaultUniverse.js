...
const CATEGORY_MAP = { Vehicles: 'Vehicle', Architecture: 'Architecture', Creatures: 'Creature', Characters: 'Character', Artifacts: 'Artifact', Nature: 'World', 'Sci-Fi': 'Vehicle' };
...
export default function VaultUniverse() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      setIsMobile(mobile);
      if (!mobile) requestAnimationFrame(() => setHeroReady(true));
      else setHeroReady(false);
    };
    update();
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const observer = new IntersectionObserver((entries) => entries[0]?.isIntersecting && loadMore(), { rootMargin: '300px' });
    observer.observe(el);
  ...
  const visible = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      const categoryMatch = category === 'All' || item.type === mapped || (category === 'Nature' && (item.type === 'World' || item.type === 'Nature')) || (category === 'Sci-Fi' && ['ship', 'satellite', 'mech'].includes(item.shape));
  ...
  async function connectWallet() {
    try {
      if (!window.ethereum) { setStatus('MetaMask not detected. Install MetaMask or open in MetaMask Mobile.'); return; }
  ...
  async function payCrypto() {
    if (!selected) return;
    setBusy(true); setStatus(`Buying ${selected.name} with crypto…`);
    try {
      if (!hasContracts()) throw new Error('Contract addresses not found in this deploy. Add NEXT_PUBLIC_VOXEL_NFT_ADDRESS and NEXT_PUBLIC_VOXEL_MARKET_ADDRESS in Vercel env.');
      await buyAsset(selected.id, selected.price);
      setStatus(`Purchase submitted for ${selected.name}. Confirm in your wallet.`);
    } catch (e) { setStatus(e?.shortMessage || e?.reason || e?.message || 'Crypto purchase failed.'); }
    finally { setBusy(false); }
  }

  async function payCard() {
    if (!selected) return;
    setBusy(true); setStatus(`Opening Stripe checkout for ${selected.name}…`);
    try {
      const res = await fetch('/api/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ assetId: String(selected.id) }) });
    ...
    } catch (e) { setStatus(e?.message || 'Card checkout failed. Confirm STRIPE_SECRET_KEY is set in Vercel.'); }
    finally { setBusy(false); }
  }

  const renderArtwork = (item, compact = true, interactive = false) => item.renderMode === 'voxel'
    ? <VoxelViewer shape={item.shape} material={item.material} seed={item.seed} rarity={item.rarity} compact={compact} showcase={!interactive} interactive={interactive} label={false} />
    : <ArtPreview family={item.family} material={item.material} seed={item.seed} compact={compact} interactive={interactive} showcase={!interactive} />;

  const renderFallback = (item, featured = false) => (
    <div className={`cardPlaceholder ${featured ? 'featuredPlaceholder' : ''}`} aria-hidden="true">
      <div className="placeholderOrb"><span>{item.family || item.shape || '3D'}</span></div>
      <strong>{item.name}</strong>
      <small>{item.rarity} · {item.material || item.style || 'Digital Object'}</small>
      {!featured && <span className="phHint">Tap to open 3D</span>}
    </div>
  );

  return (
    <main className="vaultUniverse">
      <div className="universeGlow glowA" /><div className="universeGlow glowB" />
      ...
      <section className="hero" id="discover">
        <div className="heroCopy">
          ...
          <div className="heroActions"><a href="#drops" className="primaryAction">Explore the Vault ↓</a><a href="#creators" className="secondaryAction">Create a drop →</a></div>
        ...
        <div className="heroViewer">
          {heroReady && !isMobile && !selected ? <VoxelViewer shape="dragon" material="crystal" rarity="Mythic" seed="hero-obsidian-dragon" showcase interactive={false} label={false} /> : renderFallback({ name: 'Obsidian Dragon', family: 'Dragon', rarity: 'Mythic', material: 'Crystal' }, true)}
      ...
      <section className="discover" id="drops">
        <div className="sectionHead"><div><div className="eyebrow">THE COLLECTION</div><h2>More than one kind of <em>beautiful.</em></h2></div><p>{CATALOG_SIZE.toLocaleString()} forms — lightweight previews keep browsing fast. Open any piece for full inspection.</p></div>
        <div className="toolbar"><div className="categoryBar">{CATEGORIES.map((item) => <button key={item} className={category === item ? 'selected' : ''} onClick={() => setCategory(item)}>{item}</button>)}</div><label className="search"><span>⌕</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search objects, creators, styles..." /></label></div>
        <div className="gallery">
          {visible.map((item, index) => {
            const live3d = !isMobile && index === 0 && !selected;
            return <article className="artCard" key={`${item.id}-${item.seed}`}>
              <button className="artVisual" onClick={() => setSelected(item)} aria-label={`Inspect ${item.name}`}>
                {live3d ? renderArtwork(item, true, false) : renderFallback(item)}
              ...
              <div className="artMeta"><span>{item.type}</span><span>{item.style || item.material}</span><button onClick={() => setSelected(item)}>Inspect 3D ↗</button></div>
            </article>;
          })}
        </div>
