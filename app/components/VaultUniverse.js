...
export default function VaultUniverse() {
  ...
  const visible = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      const categoryMatch = category === 'All' || item.type === mapped || (category === 'Nature' && (item.type === 'World' || item.type === 'Nature')) || (category === 'Sci-Fi' && (item.shape === 'ship' || item.shape === 'satellite' || item.shape === 'mech'));
  ...
  function payCrypto() {
    if (!selected) return;
    setBusy(true);
    setStatus(`Preparing crypto purchase for ${selected.name}…`);
    setTimeout(() => {
      setStatus(`Crypto path ready. Connect wallet and confirm when contracts are live. Price: ${selected.price} ETH`);
      setBusy(false);
    }, 500);
  }
  ...
  function payCard() {
    if (!selected) return;
    setBusy(true);
    setStatus(`Preparing card checkout for ${selected.name}…`);
    setTimeout(() => {
      setStatus(`Card path ready via Stripe. Estimated $${selected.priceUsd || '—'} USD.`);
      setBusy(false);
    }, 500);
  }
    ...
    <main className="vaultUniverse">
      ...
      <section className="hero" id="discover">
        <div className="heroCopy">
          ...
          <p>Explore original voxel sculptures, detailed environments, creatures, machines and impossible worlds. Pay with crypto or card. Inspect every object in real 3D before you collect it.</p>
        ...
        <div className="heroViewer">
          ...
          <div className="heroTag"><span>FEATURED</span><strong>Obsidian Dragon</strong><small>Mythic · Crystal · denser geometry</small></div>
        </div>
      </section>
      ...
      <section className="discover" id="drops">
        <div className="sectionHead">
          ...
          <p>From vehicles and architecture to creatures, relics and science-fiction machines. Endless deterministic vault — only a window of cards loads at a time so the browser stays fast.</p>
        </div>
        ...
        <div ref={sentinelRef} className="loadMore">
          {loadedCount < CATALOG_SIZE
            ? `Loading more from the vault… ${loadedCount.toLocaleString()} / ${CATALOG_SIZE.toLocaleString()}`
            : `End of current window · ${CATALOG_SIZE.toLocaleString()} forms available`}
        </div>
      ...
      <section className="manifesto" id="about">
        ...
        <div className="manifestoGrid">
          <div><b>01</b><strong>Real 3D</strong><p>Large interactive previews put geometry first. GLB/GLTF assets can become the centerpiece instead of an afterthought.</p></div>
          <div><b>02</b><strong>Original variety</strong><p>Micro-voxel sculpture, realistic machines, architecture, creatures, fantasy, sci-fi, nature and abstract work can coexist.</p></div>
          <div><b>03</b><strong>Pay your way</strong><p>Crypto via wallet or card via Stripe. The interface never pretends a purchase happened until it actually does.</p></div>
        </div>
      </section>
      ...
      <section className="creatorBanner" id="creators">
        <div>
          ...
          <p>Stage an original GLB/GLTF asset, attach metadata and royalties, then connect the real contract layer when the production network is configured.</p>
        </div>
        ...
        <div className="inspectBackdrop" role="dialog" aria-modal="true">
          <div className="inspectModal">
            ...
            <div className="inspectDetails">
              ...
              <h2>{selected.name}</h2>
              <p>{selected.description || selected.style}. Created by <b>{selected.creator}</b>. Rotate, zoom and inspect before you collect.</p>
              ...
              <div className="payRow">
                <button className="collect" onClick={payCrypto} disabled={busy}>Pay with Crypto</button>
                <button className="cardPay" onClick={payCard} disabled={busy}>Pay with Card</button>
              </div>
