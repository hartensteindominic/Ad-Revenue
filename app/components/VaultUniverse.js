...
// Keep concurrent WebGL scenes low to avoid browser freeze
const PAGE_SIZE = 8;
...
export default function VaultUniverse() {
  ...
  const loadMore = useCallback(() => {
    ...
    // Cap how many cards stay in memory to protect the browser
    const next = getCatalogWindow(loadedCount, PAGE_SIZE);
    setItems((prev) => {
      const merged = [...prev, ...next];
      // Keep only the most recent window of cards (~24) so old WebGL scenes can unmount
    ...
    <main className="vaultUniverse">
      ...
      <section className="discover" id="drops">
        <div className="sectionHead">
          ...
          <p>From vehicles and architecture to creatures, relics and science-fiction machines. {CATALOG_SIZE.toLocaleString()} forms — only a few 3D previews stay live so the page stays fast.</p>
        </div>
        ...
        <div className="gallery">
          {visible.map((item, index) => {
            // Only mount live 3D for the first MAX_LIVE_VIEWERS cards to protect GPU
      ...
      <section className="manifesto" id="about">
        ...
        <div className="manifestoGrid">
          ...
          <div><b>03</b><strong>Pay your way</strong><p>Crypto via wallet or card via Stripe when connected.</p></div>
        </div>
      </section>
