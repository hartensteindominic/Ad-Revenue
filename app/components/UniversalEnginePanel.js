...
import CollectiblePreview from './CollectiblePreview';

const FAMILY_NAMES = {
  vehicles: ['Aero', 'Drift', 'Vector', 'Nomad'], technology: ['Quantum', 'Signal', 'Atlas', 'Circuit'],
  fashion: ['Chrome', 'Velvet', 'Prism', 'Lumen'], sports: ['Velocity', 'Pulse', 'Orbit', 'Apex'],
  architecture: ['Monolith', 'Arc', 'Habitat', 'Spire'], nature: ['Ember', 'Frost', 'Bloom', 'Grove'],
  creatures: ['Obsidian', 'Solar', 'Lunar', 'Titan'], artifacts: ['Ancient', 'Relic', 'Echo', 'Vault'],
  science: ['Muon', 'Nova', 'Helix', 'Vector'], scifi: ['Stellar', 'Void', 'Neon', 'Orbital'],
  fantasy: ['Mythic', 'Rune', 'Astral', 'Dragon'], furniture: ['Modular', 'Cobalt', 'Arc', 'Foundry'],
...
function buildObject({ family, prefix, noun, rarity, seed }) {
  const collectible = createUniversalCollectible({
    name: `${prefix} ${noun}`, family, subtype: family, creationMode: 'procedural', rarity, seed,
    description: `A procedurally generated ${family} collectible from the Voxel Vault universal engine.`,
    realityBasis: { inspiredBy: `${family} design language`, plausibility: family === 'fantasy' || family === 'scifi' ? 'fictional' : 'realistic' },
    traits: [
      { trait_type: 'Family', value: family }, { trait_type: 'Rarity', value: rarity }, { trait_type: 'Generation', value: 'Procedural' },
    ],
  });
...
function generateObject() {
  ...
  return buildObject({ family, prefix: randomFrom(FAMILY_NAMES[family]), noun: randomFrom(OBJECT_WORDS), rarity: randomFrom(RARITIES), seed: `${Date.now()}-${Math.random().toString(36).slice(2)}` });
}
...
export default function UniversalEnginePanel() {
  ...
  function generate() { setObjects(previous => [generateObject(), ...previous].slice(0, 6)); }
  ...
  return <section className="enginePanel" aria-label="Universal collectible engine">
    <div className="enginePanelTop">
      ...
      <div className="engineActions">
        <button type="button" className="generateButton" onClick={generate}>✦ Generate 3D NFT</button>
        <button type="button" className="engineToggle" onClick={() => setOpen(value => !value)}>{open ? 'Hide engine' : 'Inspect engine'} ↗</button>
      </div>
    </div>
    <div className="engineObjects">
      {objects.map(object => {
        const validation = validateUniversalCollectible(object);
        return <article className="engineObject" key={object.fingerprint}>
          <CollectiblePreview family={object.family} rarity={object.rarity} seed={object.seed} />
