...
export default function PremiumMarketplace() {
  ...
  const filtered = useMemo(() => {
    ...
    let result = items.filter((item) => {
      ...
      const matchesQuery = !q || `${item.name} ${item.creator} ${item.type} ${item.rarity} ${item.material || ''} ${item.style || ''}`.toLowerCase().includes(q);
  ...
  function buyCrypto() {
    if (!selected) return;
    transact(`Buying ${selected.name} with crypto…`, async () => {
  ...
  function buyCard() {
    if (!selected) return;
    setBusy(true);
    setStatus(`Preparing card payment for ${selected.name}…`);
    // Stripe checkout path — uses existing Stripe infrastructure when configured
    setTimeout(() => {
      setStatus(`Card checkout for ${selected.name} is ready when Stripe is fully connected. Estimated $${selected.priceUsd} USD.`);
      setBusy(false);
    }, 600);
  }
      ...
      <div className="marketShell">
        ...
        <div className="marketHero">
          <div>
            ...
            <p>A 3D-first art universe. Pay with crypto or card. Endless deterministic vault · {CATALOG_SIZE.toLocaleString()} forms.</p>
          </div>
          <div className="heroObject">
            ...
            <div className="heroObjectMeta">
              ...
              <strong>Obsidian Dragon</strong>
              <small>Mythic · Crystal · 0.340 ETH</small>
            </div>
          </div>
        </div>
        ...
        {tab === 'Discover' && (
          <>
              ...
              {filtered.map((item) => (
                <button key={item.id} className={`marketCard ${item.color}`} onClick={() => setSelected(item)}>
                  ...
                  <div className="cardInfo">
                    ...
                    <div className="cardPrice"><small>FROM</small><b>{item.price} ETH</b><em>~${item.priceUsd}</em></div>
                  </div>
                  <div className="cardFoot"><span>{item.style} · {item.type}</span><i>View 3D ↗</i></div>
                </button>
              ))}
            </div>
            ...
            <div className="marketFeatures">
              <div><span>01 · PAY YOUR WAY</span><strong>Crypto or card.</strong><p>Collectors can buy with ETH via wallet or with regular money through Stripe when connected.</p><button onClick={connect}>Connect wallet →</button></div>
              <div><span>02 · CREATOR ECONOMY</span><strong>Publish your world.</strong><p>Bring a GLB/GLTF model, define its price and royalty, and prepare it for ownership.</p><button onClick={() => setTab('Creators')}>Open Creator Studio →</button></div>
              <div><span>03 · SECONDARY MARKET</span><strong>Trade beyond the mint.</strong><p>Offers and auctions give collectors more ways to discover and transact.</p><button onClick={() => setTab('Offers')}>Explore market →</button></div>
            </div>
          </>
        )}

        {tab === 'Collections' && <div className="collectionsView"><div className="collectionsIntro"><span>CURATED WORLDS</span><h3>Built by creators,<br /><em>collected by you.</em></h3><p>Explore vehicles, creatures, architecture, characters and artifacts. Endless deterministic catalog of {CATALOG_SIZE.toLocaleString()} forms.</p></div><div className="collectionBands"><div><strong>GENESIS</strong><span>Endless showcase stream</span><b>01</b></div><div><strong>VOXEL WORLDS</strong><span>Architecture + environments</span><b>02</b></div><div><strong>CREATURE LAB</strong><span>Living voxel studies</span><b>03</b></div></div></div>}

        {tab === 'Offers' && <div className="actionPanel"><div className="actionCopy"><span>NEGOTIATE</span><h3>Make the object<br /><em>yours.</em></h3><p>Select any asset in Discover, then make a time-limited ETH offer.</p><button onClick={() => setTab('Discover')}>Choose an asset →</button></div><div className="actionForm"><label>OFFER AMOUNT <input value={offer} onChange={(e) => setOffer(e.target.value)} placeholder="0.080" inputMode="decimal" /></label><button disabled={!selected || busy} onClick={sendOffer}>{selected ? `Offer ${selected.name} →` : 'Select an asset first'}</button><small>{hasContracts() ? 'Marketplace contract detected.' : 'Collector preview. Live contract actions activate when configured.'}</small></div></div>}

        {tab === 'Auctions' && <div className="actionPanel"><div className="actionCopy"><span>LIVE BIDDING</span><h3>Compete for<br /><em>rare pieces.</em></h3><p>Choose an auction asset and submit a wallet-signed ETH bid.</p><button onClick={() => setTab('Discover')}>Browse assets →</button></div><div className="actionForm"><label>BID AMOUNT <input value={bid} onChange={(e) => setBid(e.target.value)} placeholder="0.150" inputMode="decimal" /></label><button disabled={!selected || busy} onClick={sendBid}>{selected ? `Bid on ${selected.name} →` : 'Select an asset first'}</button><small>Auctions use the marketplace contract when configured.</small></div></div>}

        {tab === 'Creators' && <div className="creatorStudio"><div className="creatorCopy"><span>CREATOR STUDIO</span><h3>Make the model.<br /><em>Keep the upside.</em></h3><p>Stage your real 3D file, set a primary price and royalty, then sign the mint when ready.</p><div className="studioSteps"><span><b>01</b> Model</span><span><b>02</b> Metadata</span><span><b>03</b> Royalty</span><span><b>04</b> Mint</span></div></div><div className="studioForm"><input value={creatorName} onChange={(e) => setCreatorName(e.target.value)} placeholder="Asset name" /><input value={creatorPrice} onChange={(e) => setCreatorPrice(e.target.value)} placeholder="Primary price · ETH" inputMode="decimal" /><label className="fileDrop">{creatorFile ? `✓ ${creatorFile.name}` : 'Drop a GLB / GLTF or choose a file'}<input type="file" accept=".glb,.gltf,model/gltf-binary,model/gltf+json" onChange={(e) => setCreatorFile(e.target.files?.[0] || null)} /></label><div className="royalty"><span>ROYALTY</span><input type="range" min="0" max="10" step="0.5" value={creatorRoyalty} onChange={(e) => setCreatorRoyalty(e.target.value)} /><b>{creatorRoyalty}%</b></div><button disabled={busy} onClick={publish}>{busy ? 'Waiting for wallet…' : 'Prepare mint + listing →'}</button></div></div>}
          ...
          <div className="collectorModal">
            ...
            <div className="collectorInfo">
              ...
              <div className="collectorPrice">
                ...
                <small>≈ ${selected.priceUsd} USD · pay with crypto or card</small>
              </div>
              <div className="collectorActions dual">
                <button onClick={buyCrypto} disabled={busy}>Pay with Crypto</button>
                <button className="cardBtn" onClick={buyCard} disabled={busy}>Pay with Card</button>
              </div>
              <div className="collectorSecondary">
              ...
              <div className="collectorDNA">
                ...
                <b>{selected.seed} · {selected.style} · {selected.material}</b>
              </div>
            </div>
          </div>
        </div>
