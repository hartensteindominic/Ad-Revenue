...
import { loadAIMemory, saveAIMemory } from '../../lib/ai/memory';

const INITIAL_MESSAGE = { role: 'assistant', content: 'Vault AI is online. I can analyze activity, spot quality signals, and prepare bounded recommendations.' };
const DEMO_EVENTS = [
...
export default function VaultAICopilot() {
  ...
  useEffect(() => {
    const stored = loadAIMemory();
    if (stored.length) setMessages(stored);
    setHydrated(true);
  }, []);

  useEffect(() => {
    if (hydrated) saveAIMemory(messages);
  }, [messages, hydrated]);

  const prompts = useMemo(() => ['Analyze the Vault', 'What looks hot?', 'Check for quality risks'], []);

  async function run(prompt = '') {
    if (busy || cycle >= 3) return;
    setBusy(true);
    setNotice('Processing bounded cycle…');
    const nextUserMessage = prompt ? { role: 'user', content: prompt.slice(0, 600) } : null;
    const nextConversation = [...messages, ...(nextUserMessage ? [nextUserMessage] : [])].slice(-12);
    if (nextUserMessage) setMessages(prev => [...prev, nextUserMessage]);

    try {
      const response = await fetch('/api/ai/loop', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ events: DEMO_EVENTS, cycle: cycle + 1, conversation: nextConversation }),
      });
  ...
  return <section className="aiCopilot" aria-label="Vault AI Copilot">
    ...
    <div className="aiBody">
      ...
      <div className="aiRail"><div className="railLabel">QUICK ANALYSIS</div>{prompts.map(prompt => <button key={prompt} onClick={() => run(prompt)} disabled={busy || cycle >= 3}>{prompt} ↗</button>)}<div className="aiSignal"><span>●</span><div><b>{notice}</b><small>{insights.length ? `${insights.length} signals detected this cycle` : 'No signals loaded yet'}</small></div></div></div>
    </div>
    <div className="aiComposer"><input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} placeholder={cycle >= 3 ? 'Cycle limit reached · refresh for a new session' : 'Ask the Vault AI…'} maxLength={600} disabled={busy || cycle >= 3} /><button onClick={send} disabled={busy || cycle >= 3 || !input.trim()}>Send</button></div>
    <div className="aiFoot"><span>🔒 No autonomous fund transfers</span><span>⛓️ Ownership stays on-chain</span><span>✓ Human approval for sensitive actions</span><span>💾 Conversation memory stays on this device</span></div>
