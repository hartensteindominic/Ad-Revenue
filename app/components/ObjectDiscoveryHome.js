...
function ObjectCard({ item }) {
  ...
  return (
    <article className="card">
      ...
      <div className="meta">
        <div className="title"><small>{item.type}</small><h3>{item.name}</h3><p>{item.creator}</p></div>
        <strong>{item.price}<i> ETH</i></strong>
      </div>
      <div className="physicalLine">
        <span>📦 Physical</span><span>🔐 NFT</span><span>▣ QR</span>
      </div>
      <div className="actions">
        <Link className="buy" href={options[0].href}>{physical.delivery ? 'Buy + deliver' : 'Collect'}</Link>
        <Link className="view" href={`/marketplace?asset=${encodeURIComponent(item.id)}&mode=digital`}>NFT</Link>
        {physical.temuEnabled && <a className="shop" href={physical.temuUrl} target="_blank" rel="noopener noreferrer">Shop</a>}
      </div>
    </article>
  );
}
...
export default function ObjectDiscoveryHome() {
  ...
  return (
    <main className="home">
      ...
      <section className="hero"><small>FIND · OWN · KEEP</small><h1>Objects worth <em>finding.</em></h1><p>Real things. 3D identity. One collection.</p></section>
      ...
      <section className="callout"><small>PHYSICAL + DIGITAL</small><h2>Buy the thing.<br /><em>Keep its NFT.</em></h2><p>Your object can arrive with a QR code linking the physical piece to its digital identity.</p><Link href="/marketplace">Shop the Vault →</Link></section>
