...
function BuyBoth({ item }) {
  ...
  return <div style={{ display: 'grid', gap: 8, padding: '0 14px 14px' }}>
    {ready ? <Link href={`/marketplace?purchase=${encodeURIComponent(item.purchaseAssetId)}`} style={{ textAlign: 'center', padding: '13px 10px', borderRadius: 12, background: '#fff', color: '#080a11', textDecoration: 'none', fontSize: 10, fontWeight: 900, letterSpacing: '.06em' }}>BUY PHYSICAL + 3D NFT · ${item.customerPriceUsd}</Link> : <div style={{ textAlign: 'center', padding: '13px 10px', borderRadius: 12, border: '1px solid rgba(255,255,255,.12)', color: 'rgba(255,255,255,.68)', fontSize: 10, fontWeight: 800, letterSpacing: '.06em' }}>PHYSICAL + 3D NFT · FULFILLMENT BEING VERIFIED</div>}
    <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: 12, color: 'rgba(255,255,255,.52)', fontSize: 9 }}><span>📦 ships to you</span><span>🧊 3D NFT in Vault</span><span>✦ one Voxel Vault purchase</span></div>
  </div>;
}
...
function Card({ item, index }) {
  return <article className="vv3-objectCard">
    ...
    <div className="vv3-objectDetails"><div><small>{item.type}</small><h3>{item.name}</h3></div><strong>${item.customerPriceUsd}</strong></div>
    <div className="vv3-objectMeta"><span>{item.sourceName}</span><span>3D NFT TWIN</span></div>
    <BuyBoth item={item}/>
    <div style={{ padding: '0 14px 14px', fontSize: 10, color: 'rgba(255,255,255,.5)', lineHeight: 1.5 }}>Real product source is shown for identity and provenance. Voxel Vault checkout stays on-site and is enabled only after an authorized supplier/SKU is connected. The customer price includes the configured Voxel Vault margin when supplier cost is available.</div>
  </article>;
}
