...
export default function CreateCollectible() {
  ...
  function chooseFile(nextFile) {
    ...
    if (detected !== type) {
      setStatus(`This file is detected as ${detected || 'unsupported'} media. Choose the matching tab.`);
      return;
    }
  ...
  return (
    <main className="createPage">
      ...
      <section className="shell">
        ...
        <p className="lede">3D is the flagship. Images and videos can join the same collectible and scavenger-hunt system, so anyone can drop something into the world while advanced creators build interactive objects.</p>

        <div className="typeGrid">
          {TYPES.map(item => <button key={item.id} className={type === item.id ? 'active' : ''} onClick={() => chooseType(item.id)}><b>{item.icon} {item.title}</b><span>{item.copy}</span></button>)}
        </div>
        ...
        <div className="workspace">
          <div className="preview">
            ...
            {file && type === '3d' && <div className="modelPlaceholder"><span>◇</span><b>3D ASSET STAGED</b><small>{file.name}</small><small>GLB / GLTF preview plugs into the Vault 3D viewer.</small></div>}
          </div>
          <div className="form">
