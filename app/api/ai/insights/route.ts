...
import { analyzeVaultEvents, buildAutopilotPlan, type VaultEvent } from '@/lib/ai/autopilot';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const events = Array.isArray(body?.events) ? (body.events as VaultEvent[]).slice(-500) : [];
