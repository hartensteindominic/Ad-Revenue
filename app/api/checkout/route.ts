...
export async function POST(request: Request) {
  try {
    ...
    const { data: asset, error: assetError } = await supabaseAdmin.from('assets')
      .select('id,title,price_cents,currency,seller_id,status').eq('id', String(assetId)).eq('status', 'published').single();
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ quantity: 1, price_data: { currency: asset.currency, unit_amount: asset.price_cents, product_data: { name: asset.title } } }],
      metadata: { asset_id: asset.id, buyer_id: user.id },
      payment_intent_data: { metadata: { asset_id: asset.id, buyer_id: user.id }, ...(seller?.stripe_account_id && seller.charges_enabled && seller.payouts_enabled ? { application_fee_amount: fee, transfer_data: { destination: seller.stripe_account_id } } : {}) },
