...
The preferred UX is subtle but unmistakable:
...
That reserve exists to create enough activity for the first campaigns to be interesting before sponsor revenue is meaningful.
...
Campaign revenue is divided into explicit buckets. The current engine defaults to:
...
These percentages are configuration defaults, not promises to sponsors or collectors. A production campaign must record its exact allocation before launch.

### Self-sustaining phase

When recurring sponsor revenue covers infrastructure, creator production, rewards, reserve targets and operating costs, the bootstrap contribution can fall toward zero.

The system should never assume that popularity automatically produces money. Revenue must come from measurable campaign demand, creator supply, collector activity, marketplace volume, or other explicit commercial flows.
...
That distinction matters. The success metric is not only impressions. It can include:
...
Sponsors should buy an experience and measurable participation, not a disguised banner.
...
- Campaign claims require normal anti-replay and settlement verification.
...
1. Sponsor campaign schema and allocation engine.
2. Sponsored collectible provenance.
3. Campaign funding and reserve accounting.
4. Sponsor-safe drop creation.
5. Collector reward accounting.
6. 3D sponsored-object presentation.
7. Hunt integration.
8. Sponsor analytics based on verified events.
9. Sponsor dashboard.
10. Campaign settlement and payout reconciliation.
11. Adversarial testing.
12. End-to-end review.

Do not build a full ad marketplace before the single sponsored-drop vertical slice works.
