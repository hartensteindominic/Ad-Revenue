...
export async function authorizeClaim({ dropId, walletAddress, distanceMeters = null, requireInZone = false } = {}) {
  ...
  if (db) {
    ...
    if (error) {
      ...
      if (error.code === '23505' || message.includes('already claimed')) {
        const { data: existing } = await db
          .from('voxel_claims')
          .select('status,claim_ticket,expires_at')
          .eq('drop_id', drop.id)
    ...
    return {
      ...
      expiresAt: reserved?.expires_at || null,
      dropId: drop.id,
      walletAddress: wallet,
      status: 'authorized',
      collectible: drop.collectible || null,
      ...
      nextStep: 'wallet_claim_voucher_then_chain_settlement',
      serverValidation: true,
      storage: 'supabase',
      note: 'Claim capacity was reserved atomically by the database. The signed voucher binds the claim to the wallet and exact metadata.',
    };
  }
