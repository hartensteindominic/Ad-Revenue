 ...
 *
 * When Supabase is configured, the reservation is performed by a database
 * function that locks the drop row and increments claimed_count atomically.
 * This prevents concurrent server instances from exceeding finite supply.
 */
export async function authorizeClaim({
  ...
  if (!db && memory.claims.has(claimKey)) {
  ...
  const clientDistanceMeters = Number.isFinite(distanceMeters) ? distanceMeters : null;
  ...
  if (db) {
    const { data, error } = await db.rpc('reserve_voxel_drop_claim', {
      ...
      p_client_distance_meters: clientDistanceMeters,
    });

    if (error) {
      if (error.code === '23505' || String(error.message).toLowerCase().includes('already claimed')) {
    ...
    const updatedClaimedCount = Number(reserved?.claimed_count ?? drop.claimedCount + 1);
    drop.claimedCount = updatedClaimedCount;
    ...
    return {
      ...
      serverValidation: true,
      storage: 'supabase',
      note: 'Claim reserved atomically by the server database. Ownership is not granted until a chain transaction confirms.',
    };
  }
  ...
  return {
    ...
    serverValidation: true,
    storage: 'memory',
    note: 'Claim authorized in ephemeral server memory (Supabase not configured). Not multi-instance durable.',
  };
}
