...
export async function authorizeClaim({ dropId, walletAddress, distanceMeters = null, requireInZone = false } = {}) {
  ...
  // Capacity is reserved at authorization time. This prevents concurrent claims
  // from oversubscribing a finite drop. It is not ownership and should be released
  // by a future expiry/reconciliation worker when a ticket is abandoned.
  ...
  if (db) {
    ...
    await db.from('voxel_drops').update({ claimed_count: drop.claimedCount }).eq('id', drop.id);
  }
  ...
  return {
    ...
    note: db
      ? 'Claim authorized by server. Capacity is reserved; ownership is not granted until chain confirmation.'
