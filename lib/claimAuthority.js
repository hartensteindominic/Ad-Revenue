...
function mapDrop(row) {
  return {
    ...
    claimedCount: row.claimed_count ?? row.claimedCount ?? 0,
    discovery: { publicZoneId: row.public_zone_id ?? row.discovery?.publicZoneId ?? null, radiusMeters: row.radius_meters ?? row.discovery?.radiusMeters ?? 50 },
    schedule: { startAt: row.start_at ?? row.schedule?.startAt ?? null, endAt: row.end_at ?? row.schedule?.endAt ?? null },
    claimRules: { maxClaimsPerWallet: row.max_claims_per_wallet ?? row.claimRules?.maxClaimsPerWallet ?? 1 },
    lat: row.lat,
    lng: row.lng,
...
export async function authorizeClaim({ dropId, walletAddress, distanceMeters = null, requireInZone = false, now = new Date() } = {}) {
  const wallet = normalizeWallet(walletAddress);
  const drop = await getDrop(dropId);
  if (!drop) throw new Error('Drop not found');
  if (!isDropDiscoverable(drop, now)) throw new Error('Drop is not currently active');
