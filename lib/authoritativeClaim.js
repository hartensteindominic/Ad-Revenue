...
export async function submitAuthoritativeClaim({ dropId, walletAddress, claimTicket, transactionHash }) {
  ...
  if (error) throw new Error(error.message);
  if (!data) throw new Error('Claim reservation is missing or expired');
  return { status: data.status, claimId: data.id, transactionHash: data.transaction_hash, dropId, walletAddress: address, claimTicket };
}

export async function getClaimForConfirmation({ dropId, walletAddress, claimTicket, transactionHash }) {
  const address = wallet(walletAddress);
  const database = await db();
  assertProductionStorage(database);
  const { data: claim, error } = await database.from('voxel_claims').select('id,status,transaction_hash,token_id')
  ...
  if (claim.status === 'confirmed') return { claim, database };
...
export async function confirmAuthoritativeClaim({ dropId, walletAddress, claimTicket, transactionHash, tokenId }) {
  const { claim, database } = await getClaimForConfirmation({ dropId, walletAddress, claimTicket, transactionHash });
  if (claim.status === 'confirmed') return { status: 'confirmed', alreadyFinalized: true, tokenId: claim.token_id };
  if (!tokenId) throw new Error('Verified token ID is required for confirmation');
  const { data, error } = await database.rpc('confirm_voxel_claim', { p_claim_id: claim.id, p_token_id: String(tokenId) });
  if (error) throw new Error(error.message);
  return { status: 'confirmed', dropId, walletAddress: walletAddress.toLowerCase(), claimTicket, transactionHash, tokenId: String(tokenId), confirmation: data?.[0] || null };
}
