// Commerce routing for Voxel Vault. No merchant credentials are stored in client code.
// Real merchant/affiliate destinations are configured server-side with environment variables.
...
export function getPhysicalPurchaseConfig(item) {
  ...
  return {
    ...
    temuEnabled,
    temuUrl: temuEnabled
      ? `${temuAffiliateBase}${temuAffiliateBase.includes('?') ? '&' : '?'}vv_object=${encodeURIComponent(item.id)}`
      : null,
  };
}
