const items=[
  ...
  {id:'apple-iphone-15-pro',name:'iPhone 15 Pro',creator:'Apple',type:'Phone',family:'robot',material:'metallic',rarity:'Rare',sourcePriceUsd:999,sourceName:'Apple official product source',sourceUrl:'https://www.apple.com/iphone-15-pro/',previewUri:'https://images.unsplash.com/photo-1696446701796-da61225697cc?auto=format&fit=crop&w=1200&q=85',modelSource:'Voxel Vault local 3D twin; external exact model not verified',resaleEligible:false},
  ...
  {id:'rolex-submariner',name:'Submariner',creator:'Rolex',type:'Watch',family:'artifact',material:'gold',rarity:'Mythic',sourcePriceUsd:null,priceUsd:'10000+',sourceName:'Rolex official collection source',sourceUrl:'https://www.rolex.com/watches/submariner',previewUri:'https://images.unsplash.com/photo-1524805444758-089113d48a6d?auto=format&fit=crop&w=1200&q=85',modelSource:'Voxel Vault local 3D twin; exact licensed asset not verified',resaleEligible:false},
...
export const REAL_WORLD_CATALOG=items.map((item,index)=>{
  ...
  const customerPriceUsd=priceWithMarkup(basePriceUsd,markupPercent) || item.priceUsd || null;
  return {
    ...item,
    id:item.id||`real-object-${index+1}`,
    price:item.priceUsd || (basePriceUsd ? String(basePriceUsd) : null),
    customerPriceUsd,
    basePriceUsd,
    ...
    pricingModel:'source-price-plus-vault-markup',
    markupPercent,
