/**
 * Voxel Vault physical-proximity protocol.
 *
...
export function encodeDeepLink(intent) {
  ...
  if (typeof window === 'undefined') throw new Error('Deep links require a browser');
...
export function parseProximityIntent(value, { now = Date.now(), maxAgeMs = INTENT_TTL_MS } = {}) {
  ...
  const action = String(value.action || 'discover');
  if (!ALLOWED_ACTIONS.has(action)) throw new Error('Invalid proximity action');
  ...
  return { version, action, dropId, nonce, createdAt: new Date(createdAt).toISOString() };
}
