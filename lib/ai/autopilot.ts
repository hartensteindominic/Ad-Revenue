...
export type AutopilotInsight = {
  id: string;
  kind: 'trend' | 'quality' | 'security' | 'product';
  title: string;
  summary: string;
...
export function analyzeVaultEvents(events: VaultEvent[]): AutopilotInsight[] {
  ...
  if (claims.length) {
    insights.push({
      id: 'claim-activity',
      kind: 'trend',
      title: 'Claim activity detected',
      summary: `${claims.length} claim event${claims.length === 1 ? '' : 's'} across ${uniqueWallets.size} wallet${uniqueWallets.size === 1 ? '' : 's'}.`,
      priority: claims.length >= 20 ? 'high' : 'low',
      evidence: { claims: claims.length, wallets: uniqueWallets.size },
    });
  }
  ...
  if (failureRate > 0.05) {
    insights.push({
      id: 'error-rate',
      kind: 'quality',
      title: 'Quality signal needs attention',
      summary: `${Math.round(failureRate * 100)}% of supplied events are failures.`,
      priority: failureRate > 0.2 ? 'high' : 'medium',
      evidence: { failures: failures.length, events: events.length, failureRate: failureRate.toFixed(3) },
    });
  }

  const repeatedDrops = new Map<string, number>();
  for (const claim of claims) {
    if (claim.dropId) repeatedDrops.set(claim.dropId, (repeatedDrops.get(claim.dropId) || 0) + 1);
  }
  ...
  if (hottest && hottest[1] >= 3) {
    insights.push({
      id: 'hot-drop',
      kind: 'trend',
      title: 'High-demand drop',
      summary: `Drop ${hottest[0]} is receiving concentrated claim activity.`,
      priority: hottest[1] >= 10 ? 'high' : 'medium',
      evidence: { dropId: hottest[0], claims: hottest[1] },
    });
  }
...
/**
 * Bounded autonomy: this function may recommend actions, never execute financial,
 * ownership, deployment, or destructive operations. Human/on-chain authorization
 * remains mandatory for anything that changes ownership or moves funds.
 */
export function buildAutopilotPlan(insights: AutopilotInsight[]) {
