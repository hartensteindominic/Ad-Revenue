...
function requiredRpc() {
  const value = process.env.EVM_RPC_URL || process.env.SEPOLIA_RPC_URL;
  if (!value) throw new Error('Missing server configuration: EVM_RPC_URL');
  return value;
}
...
function configuredNftAddress() {
  const address = process.env.NEXT_PUBLIC_VOXEL_NFT_ADDRESS;
  if (!address || !isAddress(address)) throw new Error('Invalid NFT contract address');
...
export async function verifyClaimTransaction({ transactionHash, walletAddress, claimTicket }) {
  ...
  const rpc = new JsonRpcProvider(requiredRpc(), Number(chainId));
  ...
  let decoded;
  try { decoded = iface.parseTransaction({ data: tx.data, value: tx.value }); } catch { return { verified: false, reason: 'not_a_voxel_mint_transaction' }; }
  ...
  for (const log of receipt.logs || []) {
    ...
    if (log.topics?.[0]?.toLowerCase() === TRANSFER_TOPIC && log.topics.length >= 4) {
      ...
      if (from === ZERO && to === wallet) { tokenId = BigInt(log.topics[3]).toString(); recipientFound = true; }
    }
    try {
  ...
  return { verified: true, chainId: chainId.toString(), transactionHash, contractAddress: nftAddress, walletAddress: wallet, tokenId, blockNumber: receipt.blockNumber };
}
