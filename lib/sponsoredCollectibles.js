/**
 * Sponsored Collectibles Engine
 *
 * A sponsor-funded collectible is still a real Voxel Vault object.
 * Sponsorship pays for discovery, rewards, hosting and creator economics.
 * Sponsorship MUST be disclosed in user-facing metadata/UI. The collectible
 * may be art-first and brand-native, but it must never masquerade as
 * unrelated organic content.
 */
...
export function createSponsoredCampaign(input = {}) {
  ...
  const total = creatorShare + collectorRewardShare + platformShare + reserveShare;
  if (Math.abs(total - 1) > 0.000001) throw new Error('Campaign allocation ratios must total 1');
...
export function calculateCollectorReward({ campaign, completedClaims = 0, maxClaims = 1 } = {}) {
  ...
  return {
    reward,
    remainingPool: money(Math.max(0, pool - reward * completed)),
    capacity,
