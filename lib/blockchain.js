...
// Production is Ethereum Mainnet by default. Test networks can be selected explicitly
// through environment variables without changing the marketplace UI.
export const EVM_CHAIN_ID = process.env.NEXT_PUBLIC_EVM_CHAIN_ID || '0x1';
export const EVM_CHAIN_ID_DECIMAL = BigInt(EVM_CHAIN_ID);
export const EVM_CHAIN_NAME = process.env.NEXT_PUBLIC_EVM_CHAIN_NAME || 'Ethereum Mainnet';
export const EVM_NATIVE_SYMBOL = process.env.NEXT_PUBLIC_EVM_NATIVE_SYMBOL || 'ETH';
export const EVM_EXPLORER_URL = process.env.NEXT_PUBLIC_EVM_EXPLORER_URL || 'https://etherscan.io';
...
export async function mintAndList({ uri, royaltyPercent, priceEth }) {
  if (!hasContracts()) throw new Error('Voxel Vault Ethereum contracts are not configured yet.');
