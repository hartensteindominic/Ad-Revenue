...
export function generateVoxelAsset({ shape = 'car', seed = 'voxel-vault-1', rarity = 'Rare', material } = {}) {
  ...
  // rarity-driven detail density
  const detailCount = level * 8 + Math.floor(random() * level * 7);
  for (let i = 0; i < detailCount; i++) {
    const x = Math.floor(random() * 17) - 8;
    const y = Math.floor(random() * 14) - 2;
    const z = Math.floor(random() * 13) - 6;
    addUnique(x, y, z, Math.floor(random() * Math.min(5, 2 + level)));
  }

  // Epic+ structural accents
  if (level >= 4) {
    for (let x = -6; x <= 6; x++) addUnique(x, 8, Math.round(Math.sin(x * 0.7) * 2), 4);
    for (const s of [-1, 1]) {
      addUnique(8 * s, 9, 0, 3);
  ...
  if (level >= 5) {
    for (let x = -7; x <= 7; x++) for (let z = -3; z <= 3; z++) if ((x * x + z * z) % 4 === 0) addUnique(x, 11, z, 4);
    for (const s of [-1, 1]) for (let y = 0; y < 7; y++) addUnique(10 * s, y, 0, 3);
  }

  // Mythic orbital / energy rings
  if (level >= 6) {
    for (let y = 0; y < 6; y++) {
      for (let a = 0; a < 6; a++) {
        const ang = (a / 6) * Math.PI * 2;
        const x = Math.round(Math.cos(ang) * (9 + y * 0.4));
        const z = Math.round(Math.sin(ang) * (9 + y * 0.4));
        addUnique(x, y + 2, z, 4);
      }
    }
  }

  const dna = `VV2-${shape}-${matKey}-${rarity}-${hashString(`${seed}:${shape}:${rarity}:${matKey}:${voxels.length}`).toString(16).padStart(8, '0')}`;
