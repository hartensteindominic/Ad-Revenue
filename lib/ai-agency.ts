...
const REVERSIBLE=new Set<AIAction>(['research','organize_room','draft_listing','run_quantum_simulation']);
export function decide(action:AIAction,requested:AgencyLevel='suggest'):Decision{
 ...
 return {action,level:safe&&requested==='act'?'act':requested,reason:safe?'Low-risk informational or reversible action.':'This action can affect money, ownership, listings, or blockchain state.',requiresApproval,reversible:REVERSIBLE.has(action)};
}
