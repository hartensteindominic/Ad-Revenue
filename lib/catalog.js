...
// Geometry-first art direction. Material is a second layer, never the primary source of variety.
const ART_FAMILIES = {
...
export function getCatalogItem(index) {
  ...
  const renderMode = pick(RENDER_MODES, seed + ':render') === 'voxel' ? 'voxel' : (family === 'abstract' || family === 'artifact' ? 'sculpted3d' : pick(RENDER_MODES, seed + ':render'));
...
export function getCatalogWindow(start = 0, count = 12) {
  const items = [];
  for (let i = start; i < start + count; i++) items.push(getCatalogItem(i));
  return items;
}
